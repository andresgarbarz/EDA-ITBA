...
public interface GraphService<V, E> {
	...
	// in the case of a weighted graph, E might implements the method double
	// getWeight()
	// otherwise an exception will be thrown

	// if the graph is not "multi" and there exists other edge with same source and
	// target,
	// throws exception
	...
	// removes the only edge that connects aVertex and otherVertex in a
	...
	// removes all the edges that connects aVertex and otherVertex in multi
	// graph/digraph with theEdge properties
	// removes the only edge that connects aVertex and otherVertex in a simple
	// graph/digraph,
	...
	Iterable<V> getBFS(V start);

	Iterable<V> getDFS(V start);
